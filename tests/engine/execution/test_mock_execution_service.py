...
class TestMockExecutionService(IsolatedAsyncioTestCase):
    ...
    async def test_l2_depth_initializes_queue_away_from_the_touch(self):
        ...
        assumptions = []
        self.execution_service.assumption_event.connect(
            lambda _, assumption: assumptions.append(assumption), weak=False
        )
        ...
        self.assertEqual([], self.fills)
        self.assertEqual(0.02, assumptions[0].initial_ahead_quantity)
        self.assertEqual(2, assumptions[0].fill_model_version)
